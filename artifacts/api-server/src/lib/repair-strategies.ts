...
const STRATEGY_PLANS: Record<string, Omit<RepairPlan, "strategy" | "generatedBy" | "generatedAt">> = {
  ...
  crdb_hotspot_resolution: {
    ...
    steps: [
      ...
      "If crdb_internal is restricted (Basic/Serverless plan), fall back to pg_stat_activity for active session analysis",
      "Invoke Agent Skill: crdb/schema/index-advisor (crdb_internal.index_recommendations)",
      "If index_recommendations is restricted, fall back to information_schema.statistics for existing index catalogue",
      "Generate DDL recommendations (hash-sharded index, range split point) or surface active contention sessions",
      "Schedule maintenance window for DDL application",
    ],
    preconditions: [
      "COCKROACH_CLOUD_API_KEY configured and cluster accessible via MCP",
      "On Standard/Dedicated plans: crdb_internal.cluster_contention_events and index_recommendations available",
      "On Basic/Serverless plans: pg_stat_activity and information_schema.statistics used as fallback — upgrade for full hotspot visibility",
    ],
