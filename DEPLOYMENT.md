...
**Task definition revision 12** (current — 2026-07-19 redeploy ×3): Last redeploy: repair plan now fully LLM-generated for all fields (steps, blast radius, preconditions, alternatives, expected outcome) using a contextual prompt that includes the diagnostician output and service name; static lookup table kept as fallback. Auth fix: LoginGate checks JWT expiry on mount (purges stale tokens), custom-fetch fires `cs-auth-expired` event on any 401 → gate re-locks without page reload. Previous redeploys same day: diagnostician handoff message dynamically set after tool selection (CLI vs ECS API etc.); PENDING_APPROVAL→AUTONOMOUS transition card corrected to "Human approval received". CloudFront invalidations: I15KA8XU3YGHQ4D2SO79Y64ZG7, I3PBPJAVESOKKS3JKA07ZLJLY6, I67STLXNZL1MDGCK126RIYLPOV. Previous (2026-07-18): Lambda nodejs20→22 migration. Rev 11: `ECS_DEFAULT_SERVICE=api`; rev 10: `ECS_DEFAULT_CLUSTER=cloud-surgeon`; rev 6: curl in runtime stage.
